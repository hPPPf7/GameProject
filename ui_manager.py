...
UI_AREAS = {
    ...
    "options": [pygame.Rect(296, 488 + i * 45, 184, 40) for i in range(3)],
...
def draw_main_ui(
    ...
    if sub_state == "wait":
        ...
        full_rect = UI_AREAS["options"][0].unionall(
            UI_AREAS["options"]
        )  # 合併三格成一個區域
    ...
    draw_text(
        screen,
        f"{symbol} 背包（共 {len(player['inventory'])} 項）",
        inventory_bar_rect,
        font,
        center=True,
    )
    ...
    if inventory_open:
        ...
        inventory_full_rect = pygame.Rect(
            32, bar_y - content_height, 448, content_height
        )
